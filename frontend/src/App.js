...
const CryptoPnLTracker = () => {
  ...
  const loadAPIKeys = async () => {
    try {
      const response = await axios.get(`${API}/exchange-api-keys`);
  ...
  const handleAddAPIKey = async (e) => {
    e.preventDefault();
    if (!newAPIKey.exchange_name || !newAPIKey.api_key || !newAPIKey.api_secret) {
      alert('Please fill in all fields');
      return;
    }

    try {
      const response = await axios.post(`${API}/exchange-api-keys`, newAPIKey);
      setApiKeys([...apiKeys, response.data]);
      setNewAPIKey({ exchange_name: 'kraken', api_key: '', api_secret: '' });
      alert('API Key added successfully!');
    } catch (error) {
      console.error('Error adding API key:', error);
      alert('Error adding API key: ' + (error.response?.data?.detail || error.message));
    }
  };

  const handleUpdateAPIKey = async (e) => {
    e.preventDefault();
    if (!editingAPIKey || !newAPIKey.exchange_name || !newAPIKey.api_key || !newAPIKey.api_secret) {
      alert('Please fill in all fields');
      return;
    }

    try {
      const response = await axios.put(`${API}/exchange-api-keys/${editingAPIKey.id}`, newAPIKey);
      setApiKeys(apiKeys.map(key => key.id === editingAPIKey.id ? response.data : key));
      cancelEditingAPIKey();
      alert('API Key updated successfully!');
    } catch (error) {
      console.error('Error updating API key:', error);
      alert('Error updating API key: ' + (error.response?.data?.detail || error.message));
    }
  };
  ...
  const handleDeleteAPIKey = async (keyId) => {
    ...
    try {
      await axios.delete(`${API}/exchange-api-keys/${keyId}`);
      setApiKeys(apiKeys.filter(key => key.id !== keyId));
      alert('API Key deleted successfully!');
    } catch (error) {
      console.error('Error deleting API key:', error);
      alert('Error deleting API key: ' + (error.response?.data?.detail || error.message));
    }
  };
  ...
  const startEditingAPIKey = (apiKey) => {
    ...
    setNewAPIKey({
      exchange_name: apiKey.exchange_name,
      api_key: apiKey.api_key,
      api_secret: '********' // Don't show actual secret
    });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-4 py-4">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center space-x-3">
              ...
              <button onClick={() => setShowAPIKeyManager(true)} className="btn-secondary text-sm">
                API Keys
              </button>
      ...
      {/* API Key Manager Modal - Mobile Optimized */}
      {showAPIKeyManager && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end md:items-center justify-center z-50 p-4 md:p-4">
          <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-xl w-full md:max-w-3xl md:w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-gray-900">Manage Exchange API Keys</h2>
                <button
                  onClick={() => {
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
                <h3 className="text-sm font-semibold text-blue-800 mb-2 flex items-center">
                  ℹ️ <span className="ml-1">About API Keys</span>
                </h3>
                <p className="text-blue-700 text-sm mb-2">
                  API keys allow automatic syncing of your exchange balances. They are stored securely and only used for read-only balance queries.
                </p>
                <p className="text-blue-700 text-xs">
                  <strong>Currently supported:</strong> Kraken (Spot & Futures), Binance and Bitget coming soon.
                </p>
              </div>

              {/* Add/Edit API Key Form */}
              <form onSubmit={editingAPIKey ? handleUpdateAPIKey : handleAddAPIKey} className="mb-6 p-4 border border-gray-200 rounded-xl">
                <h3 className="text-lg font-medium text-gray-900 mb-4">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      value={newAPIKey.exchange_name}
                      onChange={(e) => setNewAPIKey(prev => ({ ...prev, exchange_name: e.target.value }))}
                      className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                      required
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm"
                      placeholder={editingAPIKey ? "Leave blank to keep existing" : "Your exchange API secret"}
                ...
                <div className="mt-4 flex space-x-3">
                  ...
                  {editingAPIKey && (
                    <button type="button" onClick={cancelEditingAPIKey} className="flex-1 btn-secondary py-3">
                      Cancel
                    </button>
                  )}
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <div key={apiKey.id} className="p-4 border border-gray-200 rounded-xl">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-medium text-gray-900 capitalize">{apiKey.exchange_name}</p>
                            <p className="text-sm text-gray-500">
                              Key: {apiKey.api_key.substring(0, 8)}...{apiKey.api_key.slice(-4)}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                  ...
                  {apiKeys.length === 0 && (
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm text-gray-400 mt-1">Add your first API key above to enable automatic balance syncing!</p>
                    </div>
                  )}
                </div>
              </div>
