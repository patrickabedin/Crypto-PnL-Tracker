...
# Create a router without prefix
    ...
    try:
        ...
        for exchange in exchanges:
            ...
            if exchange_name == "kraken":
                ...
                api_key_doc = await db.exchange_api_keys.find_one({
                    "user_id": user_id,
                    "exchange_name": "kraken",
                    "is_active": True
                })
                ...
                if api_key_doc:
                    ...
                    if balance_result['success'] and balance_result['balance_eur'] > 0:
                        # Store balance in exchange document for later use
                        await db.exchanges.update_one(
                            {"id": exchange["id"], "user_id": user_id},
                            {"$set": {
                                "last_balance": balance_result['balance_eur'],
                        ...
                        sync_results.append({
                            ...
                            "success": True,
                            "balance": balance_result['balance_eur']
                        })
                    else:
                        ...
                        await db.exchanges.update_one(
                            {"id": exchange["id"], "user_id": user_id},
                            {"$set": {
                                "sync_status": "rate_limited" if "rate limiting" in error_msg.lower() else "error",
                ...
                # For other exchanges (Binance, Bitget) - not implemented yet
                sync_results.append({
                    "exchange": exchange_name,
                    "success": False,
        ...
        successful_syncs = len([r for r in sync_results if r.get("success", False)])
        
        return {
            "message": f"Sync completed! {successful_syncs}/{len(exchanges)} exchanges synced successfully",
